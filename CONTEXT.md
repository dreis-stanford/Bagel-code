...
- `G.lastTurn` holds the previous player's actions and is rendered in a persistent sidebar panel (`#last-turn-panel`, `renderLastTurn()`) for the whole of your turn — so CPU play is visible as it happens, not only via a summary you have to go looking for. The "Full round summary" button lives in that panel (the header was too easy to miss) and only appears when more than one player has acted.
- Going out no longer jumps straight to scoring: `endHand()` defers via `G._pendingEndHand` and calls `showFinalPlay()`, which freezes the table with a "See hand scores ▶" button so the winning play can actually be seen. All-CPU watch mode auto-advances instead.
- Per-hand display state (`pendingSummary`, `lastTurn`, `cpuTurnLog`, `_pendingEndHand`) is reset at deal.
...
0a. **[DESIGN REVISIT] Deuce/Joker representation — when to auto-detect, suggest, vs. require input** — over the last several sessions the wild-card declaration flow grew incrementally (run-position ambiguity detection for Jokers, then generalized to Deuces, then sets got their own silent-vs-prompted split between Jokers and Deuces) and now has several different behaviors depending on context: silent auto-detect when unambiguous, a "please specify" prompt when ambiguous in a run, a pre-filled suggestion + prompt for Jokers in a set, and fully silent auto-assignment for Deuces in a set. User wants to step back and reconsider this holistically — when should the game guess silently, when should it show an editable suggestion, and when should it require explicit input — rather than continuing to patch one case at a time. Revisit `openMeld()`/`confirmMeld()`/`valMeld()` as a whole next session with this lens.
