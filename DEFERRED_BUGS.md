...
## Re-verified — multi-human setup (2026-08-09-21)

User reported multi-human still didn't work, after it was already fixed and
re-verified in 2026-08-09-16. Re-checked the current file directly: the
blocking `alert()` is confirmed absent, `onTypeChange()` never restricts
selecting "Human" for more than one slot, and the hint text correctly shows
"✓ N humans + M CPU — pass & play." No code path found that would still
block this. Build bumped to 2026-08-09-21 regardless, so the build-marker
footer can rule out a stale-cache report (a recurring theme in this
project — see the note in the very first entry above). If this persists
after confirming the footer reads 2026-08-09-21 or later, it needs a fresh
repro with exact steps, since nothing in the code currently explains it.

---

## Resolved — mixed human+CPU games silently blocked again (2026-08-09-16)

**Found while working on an unrelated feature (The Afikomen), not reported
by the user.** The "Mix not supported" restriction that was removed in
2026-08-09-1 (when consolidated round summaries shipped, explicitly enabling
any mix of humans and CPUs) had reappeared — `startGame()` still contained
the blocking `alert('Mix not supported...')` + `return`, and the setup
screen's live hint text still showed the old warning. `CONTEXT.md` claimed
this was fixed and stayed fixed; it evidently regressed at some point during
the many subsequent edits this session, most likely from working off an
intermediate/stale copy of the file during one of the sandbox resets noted
elsewhere in this doc. Removed the blocking check and restored the correct
hint text (`✓ N humans + M CPU — pass & play.`). Not deeply re-tested beyond
confirming the block is gone and the file parses, since it's a straight
revert to already-verified 2026-08-09-1 behavior.

**Process note:** this is the second time in this project a previously-fixed
thing silently reverted (the first was the endTurnHO guard regression in
2026-08-09-11, though that one was self-inflicted in the same turn rather
than a later reversion). Worth periodically spot-checking a few "already
fixed" CONTEXT.md claims against the actual file when starting a new work
session, rather than only trusting the changelog.
